...
发布包由 ModBuildConfig 在 Release 编译时自动生成，文件名为 `SVSAPME 1.4.0-alpha.1.zip`。本次为 Ver1.4.0Alpha1 预发布版本。

安装方法：

1. 先安装 Stardew Valley 1.6.15、SMAPI 4.5.2 或更新版本。
2. 安装 `SVSAP 1.4.0-alpha.1`。
3. 解压 `SVSAPME 1.4.0-alpha.1.zip`。
4. 将解压出的 `SVSAPME` 文件夹放入 `Stardew Valley/Mods/`。
5. 通过 SMAPI 启动游戏。

## 多人联机

主机与所有客机必须安装相同版本的 SVSAP 和 SVSAPME。能源网络与机器状态由主机保存，客机操作会通过多人消息交由主机执行。Ver1.4.0Alpha1 新增的单方块酒桶/陈酿桶也由 SVSAPME 主机状态保存，建议服务器与 farmhand 同步更新。
